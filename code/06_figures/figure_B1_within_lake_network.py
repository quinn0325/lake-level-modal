"""Render Figure B1 from the complete within-lake CCM results.
...
def load_supported_edges():
    """Validate the 420-edge input and return the supported edges."""
    edges = pd.read_csv(TAB / "T5_within_lake_edges.csv")
    required = {
    ...
    if missing:
        raise ValueError(f"Missing within-lake columns: {sorted(missing)}")
    if set(VARIABLE_ORDER) != set(VARIABLES):
        raise ValueError("Figure variable order does not match config.py")
    ...
    if supported[["obs_lag", "obs_rho"]].isna().any().any():
        raise ValueError("Supported edges contain missing lag or skill values")
    if not supported["obs_rho"].between(0, 1).all():
        raise ValueError("Supported cross-map skills must lie from zero to one")
    return supported
...
def main():
    ...
    pairs = [(cause, effect) for cause in VARIABLE_ORDER
             for effect in VARIABLE_ORDER if cause != effect]
    ...
    for b in range(len(VARIABLE_ORDER)):
        if b % 2:
    ...
    for b in range(1, len(VARIABLE_ORDER)):
    ...
    for b, cause in enumerate(VARIABLE_ORDER):
